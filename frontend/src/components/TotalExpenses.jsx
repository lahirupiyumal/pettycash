...
export default function TotalExpenses({ records = [] }) {
  ...
  const monthlyExpensesData = useMemo(() => {
    ...
    records.forEach((record) => {
      ...
      const amount = Number(record.invoiceAmount) || 0;
  ...
  const hasDataForYear = monthlyExpensesData.some((item) => item.total > 0);
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mt-6">
      <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-black tracking-tight text-slate-900">Monthly Total Expenses</h3>
            <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-widest mt-0.5">Year-wise expense tracking</p>
          </div>
        </div>

        <div className="flex items-center gap-3 bg-slate-50 px-5 py-2.5 rounded-xl border border-slate-200">
          <CalendarDays className="h-4 w-4 text-orange-600" />
          <label htmlFor="expenses-year" className="text-xs font-bold uppercase tracking-widest text-slate-500">
            Year:
          </label>
          <select
            id="expenses-year"
      ...
      <div className="p-6">
        {!hasDataForYear ? (
          <div className="flex h-64 items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50">
            <p className="text-sm font-semibold text-slate-500">No expense data available for {selectedYear}.</p>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={380}>
            <BarChart data={monthlyExpensesData} margin={{ top: 16, right: 16, left: 16, bottom: 16 }}>
              ...
              />
              <Tooltip
                formatter={(value) => [Number(value).toLocaleString('en-LK'), <span className="font-semibold text-slate-500">Total Expenses</span>]}
                contentStyle={{
                  borderRadius: '16px',
                ...
                itemStyle={{ padding: '2px 0', fontWeight: 800, color: '#f97316' }}

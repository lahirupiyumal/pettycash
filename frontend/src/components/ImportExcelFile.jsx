...
import { UploadCloud, FileSpreadsheet, CloudLightning, RefreshCw } from 'lucide-react';
...
export default function ImportExcelFile({ onImportSuccess }) {
  ...
  const handleGoogleDriveSync = async () => {
    ...
    setSyncSuccess('');
    try {
      const response = await api.post('/records/google-drive-sync');
      setSyncSuccess(response.data.message);
      if (onImportSuccess) {
        onImportSuccess();
      }
    } catch (err) {
      setSyncError(err.response?.data?.message || err.message || 'Error syncing from Google Drive');
    } finally {
    ...
    <>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mt-8 max-w-4xl mx-auto">
        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between bg-white">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 text-blue-600 shadow-inner">
              <CloudLightning className="h-5 w-5" strokeWidth={2} />
            </div>
            <div>
              <h3 className="text-lg font-black text-slate-900 tracking-tight">Google Drive Synchronization</h3>
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-widest mt-0.5">Automated Cloud Sync</p>
            </div>
          </div>
        </div>
        
        <div className="p-8 bg-slate-50/50">
          <div className="mb-6 max-w-2xl text-sm font-medium text-slate-600 leading-relaxed">
            <p>
              Synchronize petty cash records directly from the shared Google Drive spreadsheet. This will pull the latest sheet data, perform automated validation, filter out duplicate records, and sync with your system.
            </p>
          </div>
          ...
          <div className="flex justify-end">
            <button 
              ...
              className="group relative inline-flex items-center justify-center gap-2 overflow-hidden rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 px-8 py-3.5 text-sm font-bold text-white shadow-lg shadow-indigo-900/20 transition-all duration-300 hover:scale-[1.02] hover:shadow-indigo-900/40 focus:outline-none focus:ring-2 focus:ring-blue-500/50 disabled:pointer-events-none disabled:opacity-50 ring-1 ring-white/10"
            >
              {syncLoading ? (
                <>
                ...
                <>
                  <RefreshCw className="h-4 w-4 transition-transform group-hover:rotate-180 duration-500" strokeWidth={2.5} />

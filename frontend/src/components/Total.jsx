...
export default function Total({ records = [] }) {
  ...
  const hasDataForYear = monthlyTotalData.some((item) => item.total > 0);
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mt-6">
      <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-black tracking-tight text-slate-900">Monthly Total</h3>
            <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-widest mt-0.5">Year-wise total amount</p>
          </div>
        </div>

        <div className="flex items-center gap-3 bg-slate-50 px-5 py-2.5 rounded-xl border border-slate-200">
          <CalendarDays className="h-4 w-4 text-teal-600" />
          <label htmlFor="total-year" className="text-xs font-bold uppercase tracking-widest text-slate-500">
            Year:
          </label>
          <select
            id="total-year"
      ...
      <div className="p-6">
        {!hasDataForYear ? (
          <div className="flex h-64 items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50">
            <p className="text-sm font-semibold text-slate-500">No total data available for {selectedYear}.</p>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={380}>
            <BarChart data={monthlyTotalData} margin={{ top: 16, right: 16, left: 16, bottom: 16 }}>
              ...
              />
              <Tooltip
                formatter={(value) => [Number(value).toLocaleString('en-LK'), <span className="font-semibold text-slate-500">Total</span>]}
                contentStyle={{
                  borderRadius: '16px',
                ...
                itemStyle={{ padding: '2px 0', fontWeight: 800, color: '#0d9488' }}

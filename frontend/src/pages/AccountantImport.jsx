...
export default function AccountantImport({ onImportSuccess }) {
  ...
  const handleGoogleDriveSync = async () => {
    ...
    setSyncSuccess('');
    try {
      const response = await api.post('/accountants/google-drive-sync');
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mt-8 max-w-4xl mx-auto">
        ...
        <div className="p-8 bg-slate-50/50">
          <div className="mb-6 max-w-2xl text-sm font-medium text-slate-600 leading-relaxed">
            <p>
              Synchronize accountant details directly from the shared Google Drive spreadsheet. This will pull the latest sheet data, perform automated validation, filter out duplicate records, and sync with your system.
            </p>
          </div>

...
export default function ForgotPassword() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      const { data } = await api.post('/password-reset/send-otp', { email: normalizedEmail });
      setSuccess(data.message);
      setTimeout(() => navigate('/verify-otp', { state: { email: normalizedEmail } }), 800);
    } catch (err) {
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="w-full overflow-hidden rounded-3xl border border-white/80 bg-white/90 shadow-2xl shadow-slate-300/40 backdrop-blur-xl">
          <div className="border-b border-slate-200/80 bg-white/70 px-8 py-7 text-center">
            ...
            <p className="mt-2 text-sm font-medium text-slate-500">Receive a 6-digit reset code</p>
          </div>
